...
class Sktm_berobat extends CI_Controller {
	...
	public function add()
	{
		...
		$this->load->view('sktm_berobat/add',$data);
	}
	public function post()
	{
		...
		$this->db->insert('sktm_berobat', $data);
		$this->session->set_flashdata('message', 'Data berhasil di input');
		redirect(site_url('sktm_berobat'));
	}
	public function getAll()
	{
		...
		$wargas = $this->Sk_domisili_model->sktm_berobat($kode_desa);
		...
		foreach($wargas->result() as $r) {
			...
			$data[] = array(
				$r->no_surat,
				$r->nama_warga,
				"<div class='text-wrap width-10'>".$r->ket_bawah."</div>",
				$r->created_at,
				"<a target='_blank' href='".site_url('sktm_berobat/cetak/').$r->id."' class='btn btn-success'>Cetak</a> ".
	...
	{
		$id=$this->input->post('id');
		$this->db->delete('sktm_berobat', ['id'=>$id]);
	}
	public function dt()
	{
		$id=$this->input->post('id');
	...
	public function cetak($id)
	{
		...
		$data['surat']=$this->db->get_where('sktm_berobat',['id'=>$id])->row_array();
		...
		$this->pdf->filename = "sktm_berobat".$data['surat']['id'].".pdf";
		$this->pdf->load_view('sktm_berobat/cetak', $data);
	}
}
